...
const App: React.FC = () => {
  ...
  const handleOpenModal = (task?: Task) => {
    if (task) {
      ...
      setFormData({
        ...
        category: 'routine',
        lastDoneDate: new Date().toISOString().split('T')[0],
        nextDueDate: calculateNextDate(new Date().toISOString().split('T')[0], 'weekly')
      });
    }
  ...
  const handleExportBackup = () => {
    ...
    link.href = url;
    link.download = `lifeflow-backup-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  ...
  const handleExportExcel = () => {
    ...
    // Gerar e Baixar
    XLSX.writeFile(workbook, `lifeflow-backup-excel-${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleImportBackup = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (confirm("ATENÇÃO: Isso substituirá todos os seus dados atuais. Deseja prosseguir com a restauração?")) {
  ...
  const completeAndArchive = (task: Task) => {
    ...
    const nextDate = calculateNextDate(today, task.frequency);
    setTasks(prev => prev.map(t => t.id === task.id ? { ...t, lastDoneDate: today, nextDueDate: nextDate, status: 'completed' } : t));
  };
  ...
  const handleAddNoteItem = (listId: string) => {
    ...
    if (!list) return;
    const isDuplicate = list.items.some(i => i.text.toLowerCase() === text.toLowerCase());
    if (isDuplicate) {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 relative">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[3rem] p-10 shadow-2xl animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-6">
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                ...
                <p className="text-sm text-slate-500 mb-6 font-medium leading-relaxed">Seus dados são armazenados localmente. Escolha abaixo como deseja proteger ou extrair suas informações.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={handleExportExcel} className="flex flex-col items-center justify-center gap-3 bg-indigo-50 p-6 rounded-2xl border-2 border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all group">
                    ...
                    <span className="text-[10px] font-black uppercase text-center">Exportar Excel (.xlsx)</span>
                  </button>
                  <button onClick={handleExportBackup} className="flex flex-col items-center justify-center gap-3 bg-white p-6 rounded-2xl border-2 border-slate-100 hover:border-indigo-200 transition-all group">
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-900 text-center">Exportar JSON (Backup)</span>
                  </button>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-black text-slate-900">Importação Legada</h4>
                  <p className="text-xs text-slate-400">Adicionar demandas em massa por texto</p>
                </div>
                <button onClick={() => { setIsSettingsOpen(false); setIsImportModalOpen(true); }} className="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-xs font-black hover:bg-indigo-600 transition-colors uppercase tracking-widest">Abrir</button>
              </div>
              
              <button onClick={() => { if(confirm("Deseja apagar todos os dados permanentemente? Isso não pode ser desfeito.")) { localStorage.clear(); window.location.reload(); } }} className="w-full py-4 text-red-400 text-xs font-black uppercase tracking-widest hover:text-red-600 transition-colors">Zerar Banco de Dados</button>
            </div>
          </div>
        </div>
      )}
      ...
      {mentalClutterActive && similarTaskFound && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[3rem] shadow-2xl p-10 border border-indigo-100 text-center animate-in zoom-in duration-500">
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Espaço Mental Ocupado</h2>
            <p className="text-slate-700 mb-8 text-lg leading-relaxed">
              A demanda <b className="text-indigo-600">"{formData.name}"</b> já pulsa na sua mente como <b className="text-slate-900">"{similarTaskFound.name}"</b>.
              <br/><br/>
              Repetições são gatilhos de estresse. Vamos resolver isso?
            </p>
            <div className="space-y-3">
              <button onClick={() => { setTasks(prev => prev.map(t => t.id === similarTaskFound.id ? { ...t, status: 'active', frequency: 'priority', nextDueDate: todayStr } : t)); setMentalClutterActive(false); setIsModalOpen(false); setViewMode('active'); }} className="w-full bg-indigo-600 text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-3 text-lg hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100"><Zap size={24} fill="currentColor" /> Resgatar e Priorizar</button>
              <button onClick={() => setMentalClutterActive(false)} className="w-full py-4 text-slate-500 font-bold hover:text-slate-800 transition-colors">Ignorar por enquanto</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-white rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in duration-300">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-14 h-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600"><FileSpreadsheet size={32} /></div>
              <div>
                <h2 className="text-2xl font-black">Importação em Massa</h2>
                <p className="text-slate-400 font-medium">Copie do Excel e cole abaixo (uma por linha)</p>
              </div>
            </div>
            <textarea value={importText} onChange={(e) => setImportText(e.target.value)} placeholder="Ex:&#10;Trocar óleo do carro&#10;Renovar seguro&#10;Checkup médico" className="w-full h-64 bg-slate-50 border-none rounded-3xl p-6 font-bold text-slate-700 placeholder-slate-200 focus:ring-2 focus:ring-indigo-100 transition-all mb-8" />
            <div className="flex gap-4">
              <button onClick={() => setIsImportModalOpen(false)} className="flex-1 font-bold text-slate-400 hover:text-slate-600">Cancelar</button>
              <button onClick={handleBulkImport} className="flex-[2] bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-indigo-100 uppercase tracking-widest text-sm hover:bg-indigo-700 transition-all">Sincronizar Lista</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in duration-200 overflow-hidden">
            <h2 className="text-2xl font-black text-slate-900 mb-8">{editingTask ? 'Ajustar' : 'Projetar'} Demanda</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <input required type="text" placeholder="O que sua mente pede?" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none font-bold text-lg" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} disabled={isCheckingSimilarity} />
              <div className="grid grid-cols-2 gap-4">
                <div className="relative">
                  <div className="flex justify-between items-center mb-1 px-1">
                    <label className="text-[10px] font-black uppercase text-slate-400">Periodicidade</label>
                    <button type="button" onClick={() => setShowAddFreq(!showAddFreq)} className="text-indigo-500 hover:text-indigo-700 p-1"><PlusCircle size={14}/></button>
                  </div>
                  {showAddFreq ? (
                    <div className="flex flex-col gap-2 p-3 bg-indigo-50 rounded-2xl border border-indigo-100 animate-in slide-in-from-top-2">
                      <input className="bg-white border-none rounded-xl px-3 py-2 text-xs font-bold" placeholder="Nome (Ex: Bienal)" value={newFreqName} onChange={(e) => setNewFreqName(e.target.value)} />
                      <input type="number" className="bg-white border-none rounded-xl px-3 py-2 text-xs font-bold" placeholder="Dias" value={newFreqDays} onChange={(e) => setNewFreqDays(e.target.value)} />
                      <div className="flex gap-2">
                        <button type="button" onClick={handleAddCustomFrequencyAction} className="flex-1 bg-indigo-600 text-white text-[10px] font-black py-2 rounded-lg">SALVAR</button>
                        <button type="button" onClick={() => setShowAddFreq(false)} className="px-2 text-slate-400"><X size={14}/></button>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="relative">
                  <div className="flex justify-between items-center mb-1 px-1">
                    ...
                    <button type="button" onClick={() => setShowAddCat(!showAddCat)} className="text-indigo-500 hover:text-indigo-700 p-1"><PlusCircle size={14}/></button>
                  </div>
                  {showAddCat ? (
                    <div className="flex flex-col gap-2 p-3 bg-slate-100 rounded-2xl border border-slate-200 animate-in slide-in-from-top-2">
                      <input className="bg-white border-none rounded-xl px-3 py-2 text-xs font-bold" placeholder="Nome (Ex: Pet)" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} />
                      <div className="flex gap-2">
                        <button type="button" onClick={handleAddCustomCategoryAction} className="flex-1 bg-slate-900 text-white text-[10px] font-black py-2 rounded-lg">SALVAR</button>
                        <button type="button" onClick={() => setShowAddCat(false)} className="px-2 text-slate-400"><X size={14}/></button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 mb-1 px-1 block">Última Realização</label>
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 mb-1 px-1 block">Próximo Vencimento</label>
              ...
              <div className="flex gap-4 pt-6">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 font-bold text-slate-400 hover:text-slate-600">Voltar</button>
                <button type="submit" disabled={isCheckingSimilarity} className="flex-[2] bg-slate-900 text-white font-black py-4 rounded-2xl tracking-widest uppercase text-sm shadow-xl hover:bg-indigo-600 transition-all">
